...
import { getCorporateSubscriptionPlanApi, handleStatusStudentSubscriptionPlan, handleUpdateStatusCorporateSubscriptionPlan } from "../../../../utils/services";
import { useNavigate } from "react-router-dom";
import DeleteStudentSubcriptionPlan from "../../../modals/DeleteStudentSubcriptionPlan";
import { ModalContext } from "../../../../Context";
import DeleteCorpSubcriptionPlan from "../../../modals/DeleteCorpSubcriptionPlan";

const CoprateSubsciptionPlan = () => {
  ...
  const fetchPlans = async () => {
    setLoading(true);
    try {
      const response = await getCorporateSubscriptionPlanApi();
  ...
  const handleToggleActive = useCallback(async (id, checked) => {
    try {
      await handleUpdateStatusCorporateSubscriptionPlan({ status: checked ? 1 : 0 }, id);
      fetchPlans();
    ...
    },
    {
      ...
      render: (plan) => (
        <div style={{ display: "flex", gap: 8 }}>
          <Button icon={<EyeOutlined />} onClick={() => nevigate(`/view-corporate-plan/${plan.id}`) } />
          <Button icon={<EditOutlined />} onClick={() =>nevigate(`/edit-corporate-plan/${plan.id}`) } />
  ...
  const handleDelete = (planID) => {
    const modal = <DeleteCorpSubcriptionPlan planID={planID} fetchPlans={fetchPlans} />;
    ...
    <div className="fancy-table-container">
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", gap: 8 }}>
        ...
        <Button
          ...
          onClick={() => nevigate("/add-corporate-plan")}
        >
          Create Subscription
        </Button>
      </div>
...
export default CoprateSubsciptionPlan;
