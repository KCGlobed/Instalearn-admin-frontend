...
import { getStudentSubscriptionPlanApi, handleStatusStudentSubscriptionPlan } from "../../../../utils/services";
...
const StudentSubscriptionPlan = () => {
  ...
  const fetchPlans = async () => {
    setLoading(true);
    try {
      const response = await getStudentSubscriptionPlanApi();
  ...
  const handleToggleActive = useCallback(async (id, checked) => {
    try {
      await handleStatusStudentSubscriptionPlan({ status: checked ? 1 : 0 }, id);
      fetchPlans();
    ...
    },
    {
      ...
      render: (plan) => (
        <div style={{ display: "flex", gap: 8 }}>
          <Button icon={<EyeOutlined />} onClick={() => nevigate(`/view-student-plan/${plan.id}`) } />
          <Button icon={<EditOutlined />} onClick={() =>nevigate(`/update-student-plan/${plan.id}`) } />
  ...
  const handleDelete = (planID) => {
    const modal = <DeleteStudentSubcriptionPlan planID={planID} fetchPlans={fetchPlans} />;
    ...
    <div className="fancy-table-container">
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", gap: 8 }}>
        ...
        <Button
          ...
          onClick={() => nevigate("/add-student-plan")}
        >
          Create Subscription
        </Button>
      </div>
